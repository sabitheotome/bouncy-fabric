...
public final class ProjectileCollision {
    ...
    public static final void onCollision(ProjectileEntity self, HitResult result, CallbackInfo c) {
        LOGGER.trace("[onCollision]: init");
        LOGGER.trace("[onCollision]: projectile = " + self.getName());

        boolean hasMinimumSpeed = hasMinimumSpeed(self);
        LOGGER.trace("[onCollision]: hasMinimumSpeed = " + hasMinimumSpeed);
        ...
        if (result instanceof BlockHitResult blockHitResult) {
            LOGGER.trace("[onCollision]: result instanceof BlockHitResult");

            boolean isInsideBlock = blockHitResult.isInsideBlock();
            LOGGER.trace("[onCollision]: isInsideBlock = " + isInsideBlock);
            ...
            LOGGER.trace("[onCollision]: block = " + blockState.getBlock().getName());

            @SuppressWarnings("deprecation")
            boolean blocksMovement = blockState.blocksMovement();
            LOGGER.trace("[onCollision]: blocksMovement = " + blocksMovement);
            ...
            LOGGER.trace("[onCollision]: side = " + side);

            changeProjectileMotion(self, side);
        } else if (result instanceof EntityHitResult entityHitResult) {
            LOGGER.trace("[onCollision]: result instanceof EntityHitResult");

            Entity entity = entityHitResult.getEntity();
            boolean isAttackable = entity.isAttackable();
            LOGGER.trace("[onCollision]: isAttackable" + isAttackable);
